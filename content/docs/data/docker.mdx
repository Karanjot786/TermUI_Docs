---
title: "Docker"
description: "List containers and stream live CPU, memory, and network stats via @termuijs/data."
---

# Docker

`@termuijs/data` provides a `docker` object that wraps the Docker CLI. It calls `docker ps` and `docker stats` and parses the JSON output. Docker must be installed and running on the host; the functions return empty results or `null` if Docker is unavailable.
...
Returns live resource stats for a single container by ID or name. Returns `null` if the container does not exist or is not running.

### Signature

```ts
function stats(id: string): DockerContainerStats | null
```
...
| `docker.stats(id)` | `id: string` | `DockerContainerStats \| null` | Live stats for one container |
...
```ts
import { docker } from '@termuijs/data'
...
Use `docker.stats` when you want a detail pane for one container.
...
function ContainerDetail({ id }: { id: string }) {
    const { data } = usePolling(
        () => Promise.resolve(docker.stats(id)),
        1000
    )

    if (!data) return <Text color="red">Container not found or not running.</Text>

    return (
        <col>
            <Text bold>Container: {id}</Text>
            <Text>CPU:    {data.cpu.toFixed(2)}%</Text>
            <Text>Memory: {data.mem.percent}%  ({data.mem.used} / {data.mem.limit} bytes)</Text>
            <Text>Net RX: {data.net.rx} bytes</Text>
            <Text>Net TX: {data.net.tx} bytes</Text>
            <Text>PIDs:   {data.pids}</Text>
        </col>
    )
}
```
...
- `docker.stats` calls block for up to 5 seconds per container. `docker.list` can take up to 10 seconds when many containers are running. Use `usePolling` at intervals of 3s or more to avoid stacking calls.
