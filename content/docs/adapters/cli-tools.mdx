---
title: "CLI Tools Adapters"
description: "Integrate command-line tooling into TermUI apps with useCommander (CLI flag parsing), chalkToTermUI (ANSI pass-through), useExeca / useShell (subprocess streaming), and useGit (git operations via simple-git)."
---

# CLI Tools Adapters

`@termuijs/adapters` ships four adapters for common CLI tooling: `useCommander` parses flags and arguments, `chalkToTermUI` passes chalk-styled strings through the terminal, `useExeca` and `useShell` run subprocesses and stream their output line by line, and `useGit` wraps `simple-git` with a typed interface for common git operations.

---

## useCommander

Parses CLI flags and arguments from a `commander` `Command` object. Call it once at startup to extract typed options and positional arguments before mounting your TermUI app.
...
```ts
function useCommander<T extends Record<string, unknown> = Record<string, unknown>>(
  program: Command,
  argv?: string[]
): CommanderResult<T>

interface CommanderResult<T extends Record<string, unknown> = Record<string, unknown>> {
  options: T
  args: string[]
}
```

- `program` — a configured `commander` `Command` instance.
- `argv` — the argument vector to parse. Defaults to `process.argv`.
- `options` — typed options bag, cast to the caller-specified generic `T`.
- `args` — positional arguments that remain after option parsing.

### Parse flags and render output

```ts
import { Command } from 'commander'
import { App } from '@termuijs/core'
import { Text } from '@termuijs/widgets'
import { useCommander } from '@termuijs/adapters'
...
const { options } = useCommander<Flags>(program)

const label = new Text(`Hello, ${options.name}`, { bold: true })
...
|---|---|---|
| `options` | `T` | Parsed options, typed to the caller-specified generic |
| `args` | `string[]` | Positional arguments after option parsing |
...
function ensureChalkInstalled(): void
```

- `chalkToTermUI` — returns the input string as-is, or with ANSI codes removed if `NO_COLOR` is set.
- `ensureChalkInstalled` — throws a clear error if `chalk` is not installed. Call this at startup to catch a missing dependency early.

### Strip ANSI when NO_COLOR is set

```ts
import chalk from 'chalk'
import { chalkToTermUI } from '@termuijs/adapters'
...
### Check chalk is installed at startup

```ts
import { ensureChalkInstalled } from '@termuijs/adapters'

// Throws immediately if chalk is not installed,
// instead of failing later when chalkToTermUI is first called.
...
| `ensureChalkInstalled` | `() => void` | Throws if `chalk` is not installed |

---

## useExeca / useShell

Runs external commands and streams their combined stdout and stderr output line by line as an async generator. `useShell` is an alias for `useExeca`.
...
const useShell: typeof useExeca
...
import { App } from '@termuijs/core'
import { Text } from '@termuijs/widgets'
import { useExeca } from '@termuijs/adapters'
...
import { App } from '@termuijs/core'
import { Text } from '@termuijs/widgets'
import { useGit } from '@termuijs/adapters'
