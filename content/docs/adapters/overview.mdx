...
description: "Connect TermUI apps to external tools, AI providers, filesystems, and services with @termuijs/adapters. Includes adapters for localStorage, conf, AI (OpenAI/Anthropic), RAG chat, commander, chalk, execa, git, GitHub, keychain, zod, and dotenv."
---
...
| RAGChat | `RAGChat` | TermUI widget: indexes a docs directory and answers questions via RAG |
| Vector Store | `LocalVectorStore` | In-process vector store for embedding-based similarity search |
| Commander | `useCommander` | Parses CLI flags and arguments from a `commander` `Command` object |
| Chalk | `chalkToTermUI` | Passes chalk-styled strings through and strips ANSI when `NO_COLOR` is set |
| Execa | `useExeca` / `useShell` | Runs subprocesses and streams output line by line |
...
- [AI & RAG](/docs/adapters/ai) — `useAI`, `LocalVectorStore`, `RAGChat`
- [CLI Tools](/docs/adapters/cli-tools) — `useCommander`, `chalkToTermUI`, `useExeca`, `useGit`
