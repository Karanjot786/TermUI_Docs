...
description: "Persist data across sessions in TermUI apps with useLocalStorage (JSON file store) and useConf (typed config backed by the conf library). Covers API signatures, read/write patterns, and TypeScript types."
---
...
```ts
import type { LocalStorageAdapter } from '@termuijs/adapters'
...
```ts
import { useConf } from '@termuijs/adapters'
...
```ts
import type { UseConfResult, SetConfValue } from '@termuijs/adapters'
