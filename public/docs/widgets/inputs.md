...
TermUI does not ship a dedicated `Form` or `Calendar` widget in the current release. Use the numeric and text widgets above together with layout primitives from `@termuijs/widgets` and focus management from `@termuijs/ui` to build form-like interfaces.
