...
description: "Complete API reference for @termuijs/store: createStore, all hook methods, batch, computed, history, signals, slices, and type exports."
lastUpdated: June 2026
---

# Store API Reference

This page covers every function and type exported from `@termuijs/store`. For concepts and patterns, see the [overview](./overview.mdx), [selectors](./selectors.mdx), and [middleware](./middleware.mdx) pages.
...
```ts
import { createStore, slices } from '@termuijs/store'
...
```ts
import { signal } from '@termuijs/store'
...
```ts
import { signal, mutate } from '@termuijs/store'
...
See the [Middleware page](./middleware.mdx#built-in-middleware) for full documentation on `logger` and `createLogger`.
