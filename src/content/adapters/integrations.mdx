...
description: "Connect TermUI apps to GitHub (repos, issues, PRs, releases via @octokit/rest), the OS keychain (secrets via keytar), Zod schemas (prompt validation), and .env files (environment variable loading via dotenv)."
...
import { useGitHub } from '@termuijs/adapters'
...
```ts
import { useKeychain } from '@termuijs/adapters'
...
```ts
import { z } from 'zod'
import { zodValidator } from '@termuijs/adapters'
...
import { z } from 'zod'
import { App } from '@termuijs/core'
import { Input } from '@termuijs/widgets'
import { zodValidator } from '@termuijs/adapters'
...
```ts
import { useDotenv } from '@termuijs/adapters'
