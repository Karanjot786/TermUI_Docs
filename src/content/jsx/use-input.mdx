---
title: "useInput"
description: "useInput handles raw keyboard input in @termuijs/jsx components. Receive every keypress as a string plus a KeyEvent object with modifier flags. For declarative bindings use useKeymap; for list navigation use useKeyboardNavigation."
...
Use `useInput` when you need custom per-key logic. For a list of named bindings, see [useKeymap](./use-keymap). For arrow-key list navigation, see [useKeyboardNavigation](#usekeyboardnavigation) below.
...
See the [Focus](./focus) page for the full focus system.
...
- [useKeymap](./use-keymap) — declarative named key bindings
- [Focus](./focus) — scope input to focused components
- [useMotion](./use-motion) — skip animations based on user preference
