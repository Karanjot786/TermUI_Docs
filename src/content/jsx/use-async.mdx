---
title: useAsync
description: "useAsync() hook in @termuijs/jsx manages async data loading inside terminal components. Returns { data, loading, error, refetch } with automatic state management. Use for fetching API data, reading files, or any async operation that should trigger a re-render when it resolves."
