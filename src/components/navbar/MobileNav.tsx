...
import {
  X, BookOpen, Code2, Compass, Layers, Box, Palette,
...
export function MobileNav({ open, onClose, onSearchOpen }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div
        className={`mobile-nav-sheet ${open ? 'mobile-nav-sheet-open' : ''}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="mobile-nav-header">
          <span className="mobile-nav-title">
            <span style={{ color: 'var(--accent)' }}>&gt;_</span> Navigation
          </span>
          <button
        ...
        <button
          ...
          type="button"
        >
          <span className="mobile-nav-search-icon">⌘K</span>
        ...
        <nav className="mobile-nav-content">
          ...
          <div className="mobile-nav-section">
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`mobile-nav-chevron ${expandedSection === 'docs' ? 'mobile-nav-chevron-open' : ''}`}
              />
            </button>
          ...
          <div className="mobile-nav-section">
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`mobile-nav-chevron ${expandedSection === 'api' ? 'mobile-nav-chevron-open' : ''}`}
              />
            </button>
            {expandedSection === 'api' && (
              <ul className="mobile-nav-list mobile-nav-grid">
                {API_LINKS.map((link) => (
                  <li key={link.href}>
                    <Link to={link.href} className="mobile-nav-link" onClick={onClose}>
